...
function EnglishTip(vocabulary, config) {
    ...
    function start_work() {
        ...
        var stop = 0;
        if (config.training_mode_domain && config.training_mode_domain.indexOf(domain) != -1) {
            // site
            stop = 1;
        ...
        if (parseInt(user_data.status_enable) && (current_timestamp > config.time || parseInt(config.left_traning_word) > 0) && (new Date().getTime() > config.time_last_traning || config.time_last_traning == undefined) && new String(config.time_break).length >= 1 && vocabulary.length >= get_constant("minimum_elements_for_training")) {
    ...
    function create_world(save_action, show_prev_word) {
        ...
        document.getElementById('wednesday_29_03_0').onmouseenter = function (e) {
            ...
            if (config.way_traning) {
                frag = "<div id=\"wednesday_29_03_1\"><div id=\"thursday_14_09_1\"><input type=\"text\" value=\"\" id=\"tuesday_03_10_0\" data-lpignore=\"true\" autocomplete=\"off\"></div></div>";
            } else {
                ...
                node.addEventListener('keydown', function (event) {
                    if (event.key === "Enter") {
                        ...
                        for (var index in ar_check_words) {
                            ...
                            if (input_text == word_inside || convert_qwerty().fromEn(input_text).toLowerCase() == word_inside || convert_qwerty().toEn(input_text).toLowerCase() == word_inside || convert_qwerty().fromEn(word_inside).toLowerCase() == input_text || convert_qwerty().toEn(word_inside).toLowerCase() == input_text) {
                                flag = 1;
                                break;
                            } else {
                                flag = 0;
